...
Typically Python 3 is already installed with the Raspberry Pi. However if for whatever reason it isn't installed just type in:
...
**Note Python 2 will work but does not support all Unicode Characters. Python3 is better but not perfect**

### Installing Python's MySQLdb (3)

Yourflix requires the use of a Database to allow us to build the data structures for Yourflix to work. And a lot of the leg work is done with python3.

To install MySQLdb type in ```sudo apt-get install python3-mysqldb```
...
Pi_TimeSheet
```
CREATE TABLE `Pi_YourFlix_Data`.`Pi_TimeSheet` (
  `Time_Log` VARCHAR(45) NOT NULL,
  `Time` DATETIME NOT NULL,
  PRIMARY KEY (`Time_Log`));
```

Adding the user pi (if you want to change it you need to change 
   - YourflixDbManager.py
   - updateDatabase.php
   - getLastUpdated.php
...
MariaDB> GRANT UPDATE ON Pi_YourFlix_Data.* TO '[INSERT_USERNAME]'@'localhost';
